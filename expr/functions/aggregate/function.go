...
func (f *aggregate) Do(ctx context.Context, e parser.Expr, from, until int32, values map[parser.MetricRequest][]*types.MetricData, getTargetData interfaces.GetTargetData) ([]*types.MetricData, error) {
	...
	fmt.Println("args[0] values: ", args[0].Values)
	...
	aggregated, err := helper.AggregateSeries(name, args, aggFunc)
	if err != nil {
		return aggregated, err
	}
	fmt.Println("Aggregated: ", aggregated)
	//for i := 0; i < len(args[0].Values); i++ {
	//	if !helper.XffValues(args, i, xFilesFactor) {
	//		args[0]
	//	}
	//}
